...
@register(models.CharField, models.SlugField, models.TextField)
...
@register(models.DecimalField, models.FloatField)
...
@register(models.FileField, models.FilePathField, models.ImageField)
def anonymise_file(instance, field_name, field, value):
    if field.null:
        return None

    raise AnonymiseError(
        "Cannot anonymise {} - can only null file fields".format(field_name)
    )


@register(models.EmailField)
...
@register(models.GenericIPAddressField)
...
@register(models.ForeignKey, models.OneToOneField)
def anonymise_relationship(instance, field_name, field, value):
    if field.null:
        return None

    raise AnonymiseError(
        "Cannot anonymise {} - can only null relationship field".format(field_name)
    )
...
def anonymise_manytomany(instance, field_name, field, value):
    raise AnonymiseError(
        "Cannot anonymise {} - cannot anonymise ManyToManyField".format(field_name)
    )
...
def anonymise_related_objects(obj, anonymised=None):
    ...
    relation_fields = [
        ...
        if (
            (field.one_to_many or field.one_to_one)
            and field.auto_created
            and not field.concrete
            and field.related_model in registry
            and isinstance(field.on_delete, ANONYMISE)
        )
    ]
